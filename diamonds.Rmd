...
L'anàlisi del dataset ha seguit força segments dels articles següents:
...
El tall és una variable categòrica descrivint la qualitat del tall, i la claredat, també. Els millors talls fan diamants més simètrics i lluminosos, mentre que la claredat es veu influenciada per micro escletxes i imperfeccions del diamant. Per tant, com menys imperfeccions tenim, millor és la claredat (`IF`) i viceversa (`I1`) ([Claredat](https://www.diamondse.info/diamonds-clarity.asp)).

Pel què fa a les dimensions, tenim `x`, `y`, `z`, `table`, `depth` i `carat`. `x`, `y` i `z` són les dimensions màximes/externes del diamant i _taula_ i _profunditat_ les mínimes/internes. La [imatge 1](#dimensions) iŀlustra bé les dimensions.

Finalment el pes d'un diamant es mesura en _carats_.
...
Però el què ens interessa avaluar és la importància de cada variable per a saber millor de què depèn el preu i respondre la pregunta de la què hem partit en el nostre anàlisi. Per a fer-ho, ho passem per la funció `relaimpo` que ens mostra la importància relativa de cada variable [[Imatge 9](#relaimpo)].
